...
In 1816, English scientist Francis Ronalds built a telegraph that needed only one "wire", although his prototype used a hard to imagine amount of wire strung in a grid between two insulators: eight miles!
...
The switch from static electricity to current is where everything fell into place for telegraphy. With static electricity, each use discharged all the stored power, which had to be regenerated. The signal was unreliable over any significant distance, and the signal quality could be affected by weather. Batteries solved these problems, and had the added benefit of being able to trigger an electromagnet at the receiving side.
